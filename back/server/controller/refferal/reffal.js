...
const Teamchecking = async (userid, level, reward) => {
  ...
  try {
    ...
    for (const user of allinviteduserid) {
      ...
      const capital = capitalrow?.[0]?.capital || 0;
    ...
    if (totalcapital >= requiredCapital) {
      ...
      await pool.execute(
        `UPDATE user_levels SET status='locked', statusText='locked'
...
const TrackerLevelUnlockedHistory = async () => {
  ...
  try {
    const [users] = await pool.execute(
      `SELECT id, userid, unlockedlevel, reward, lastcrediteddate FROM unlockedreward`
    );
    ...
    for (let user of users) {
      ...
      const [levelstatus] = await pool.execute(
        `SELECT status FROM user_levels WHERE level=? AND user_id=?`,
        [unlockedlevel, userid]
      );

      if (!levelstatus.length) continue;
      ...
      if (status === "locked") {
        ...
        for (let invitedid of allinvusers) {
          ...
          const capital = capitalrow?.[0]?.capital || 0;
          totalinvusercapital += Number(capital);
        }

        const requiredcapital = levelCapital[unlockedlevel];
        if (!requiredcapital) continue;

        if (totalinvusercapital >= requiredcapital) {
          console.log("qualified");
          await pool.execute(
            `UPDATE user_levels SET status='completed', statusText='completed'
             WHERE user_id=? AND level=?`,
        ...
        await pool.execute(
          "UPDATE `unlockedreward` SET `lastcrediteddate`=? WHERE id=? AND userid=?",
          [today, user.id, userid]
        );
      }
    }
  } catch (err) {
...
nodecron.schedule("0 0 * * 0",async()=>{
    await TrackerLevelUnlockedHistory()
},{
    timezone:"africa/kigali"
})
